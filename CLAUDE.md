...
- `common.py` — **4,700行。ロジックは全部ここ**（下の地図を参照）
...
| 行 | 内容 |
|---|---|
| 24-229 | インメモリキャッシュ、集計キャッシュ、各種設定定数 |
| 230-775 | スプレッドシート接続、各シートのload/save |
| 776-856 | 画像解析（Gemini） |
| 857-980 | URLから本文テキストを取る（`tools/store_collect.py` 用） |
| 981-1440 | 設定推測ロジック（機種データは `find_machine_rule` → `machine_info.to_rule`） |
| 1441-1573 | 分析結果へのQ&A（セッション単位のチャット） |
| 1574-1672 | 期待値計算（天井/ゾーン狙い） |
| 1673-2415 | 店舗の傾向分析（自分の実戦記録から集計） |
| 2416-2631 | 店舗の年間データ（外部集計の登録・読み込み） |
| 2632-2907 | ホールデータの貼り付け取り込み |
| 2908-3307 | 旧イベント日・周年日の解釈 |
| 3308-3743 | 台別データ（機種・台番号ごと）の貼り付け取り込み |
| 3744-4231 | 取り込みログ（取り込みの取り消し） |
| 4232-4639 | イベントカレンダー（月別） |
| 4640-4736 | 実戦チャット（機種情報JSONを前提に打ちながら相談） |
