...
【各項目の意味】
  ...
  admin_only  管理者にだけ出す(ページ自体の制限は各ルートの admin_required で別にかけている)
"""
    ...
    },
    {
        ...
        "items": [
            ...
            },
            {
                ...
                # ホールデータは全員で共有しているので、書き込みは管理者だけにする
                "admin_only": True,
            },
            {
...
def visible_items(category, is_admin=False):
    """メニューに表示する項目だけを返す(管理者向けの項目は is_admin のときだけ)"""
    return [
        item for item in category["items"]
        if not item.get("hidden") and (is_admin or not item.get("admin_only"))
    ]
